...
void Hit::Forward(HMM* q, HMM* t)
{
  ...
  double Cshift = pow(2.0,1.0*shift);   // score offset transformed into factor in lin-space
...
//    scale[i+1] = 1.0;   // to debug scaling
  ...
  if (loc) 
    {
	score=score-log(1.0*t->L*q->L)/LAMDA+14.; // +14.0 to get approx same mean as for -global
    }
