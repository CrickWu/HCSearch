...
//-------------- header definition ----------//
int s2a[]={ 0, 4, 3, 6,13, 7, 8, 9,11,10,12, 2,14, 5, 1,15,16,19,17,18,20};
...
//  for (int i=0; i<maxres_; i++) p[i]=(float*) memalign(16,NAA*sizeof(float));  // align memory on 16b boundaries for SSE2
//  for (int i=0; i<maxres_; i++) tr[i]=(float*) memalign(16,NTRANS*sizeof(float));
//  for (int i=0; i<maxres_; i++) trr[i]=(float*) memalign(16,NTRANS*sizeof(float));
  for (int i=0; i<maxres_; i++) p[i]=new float[NAA];
...
HMM::~HMM()
{
  ...
  for (int i=0; i<maxres; i++) if (trr[i]) free(trr[i]); else break;
  delete[] f;
  delete[] g;
  delete[] p;
  delete[] tr;
...
void HMM::AddAminoAcidPseudocounts(char pcm, float pca, float pcb, float pcc)
{
 ...
 //           tau = fmin(1.0, pca/(1. + Neff_M[i]/pcb ) );
			  tau = 1.0<pca/(1. + Neff_M[i]/pcb )?1.0:pca/(1. + Neff_M[i]/pcb );
            for (a=0; a<NAA; ++a)
              p[i][a] = (1.-tau)*f[i][a] + tau * g[i][a];
          }
      else
...
//            tau = fmin(1.0, pca/(1. + pow((Neff_M[i])/pcb,pcc)));
			  tau = 1.0<pca/(1. + pow((Neff_M[i])/pcb,pcc))?1.0:pca/(1. + pow((Neff_M[i])/pcb,pcc));
            for (a=0; a<NAA; ++a)
              p[i][a] = (1.-tau)*f[i][a] + tau * g[i][a];
          }
      break;
...
//          tau = fmax(0.0, pca*(1-x + pcc*x*(1-x)) );
		  tau = pca*(1-x + pcc*x*(1-x))>0.0?pca*(1-x + pcc*x*(1-x)):0.0;
          for (a=0; a<NAA; ++a)
            p[i][a] = (1.-tau)*f[i][a] + tau * g[i][a];
        }
