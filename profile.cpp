...
//	NewArray2D(&PSP,length,20);
//	NewArray2D(&PSM,length,20);
...
{
	delete [] residue;
//	DeleteArray2D(&PSP,length);
//	DeleteArray2D(&PSM,length);
...
/*
void PROFILE::WS_Process_PSM(ifstream &fin,string &filename,int StartPos)
{
	string wbuf;
	string temp;
	//skip first
	...
	for(int i=0;i<length;i++)
	{
		...
		for(int j=0;j<20;j++)
		{
			...
			PSM[i-StartPos][j]= -0.1*wsval;
		}
	}
}
void PROFILE::WS_Process_PSP(ifstream &fin,string &filename,int StartPos)
{
	string wbuf;
	string temp;
	//skip first
...
*/
void PROFILE::WS_Process_HMM(ifstream &fin,string &filename,int StartPos)
{
	string wbuf;
	string temp;
	//skip first
...
//				exit(-1);
		...
		{
//			fprintf(stderr,"WARNING %s -> HHM line %d low normalized %f \n",filename.c_str(),i,wssum);
			for(int j=0;j<20;j++)
			{
		...
		{
//			fprintf(stderr,"WARNING %s -> HHM line %d high normalized %f \n",filename.c_str(),i,wssum);
			for(int j=0;j<20;j++)
			{
				EmissionProb[i-StartPos][j]=0; //this bug is discovered by Jinbo
				if(WantOriginal==1)EmissionProb_original[i-StartPos][j]=0;
