...
TEMPLATE::TEMPLATE(string tempName,string root,int LoadOrig)
{
	StartPos = -1;
	tLength = -1;
	temp_name = tempName;
	...
	for(;;)
	{
		...
		if(strncmp(buf,"//////////// Features",20)==0)
		{
			...
			for(int i=0;i<length;i++)
			{
				...
				if(!(www>>temp))
				{
					fprintf(stderr,"FORMAT BAD AT TEMPLATE FILE %s -> [FEAT line %d core] \n",filename.c_str(),i);
					failure=0;
					return;
				}
				//[4] ACC
...
//					float wsval=atof(temp.c_str());  // from PSIPRED, 0 for loop, 1 for helix and 2 for sheet
				}
			}
		}//end of SS2
		//PSM
		...
		{
//			WS_Process_PSM(fin,filename,StartPos);
		}//end of PSM
		//PSP
		if(strncmp(buf,"//////////// Original PSP",25)==0)
		{
//			WS_Process_PSP(fin,filename,StartPos);
		}//end of PSP
		//HMM
		if(strncmp(buf,"//////////// Original HHM",25)==0)
		{
			WS_Process_HMM(fin,filename,StartPos);
		}//end of HHM
	}//end of for(;;)
	length = tLength;
}
